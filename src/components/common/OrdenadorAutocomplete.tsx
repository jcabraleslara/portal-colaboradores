/**
 * Componente OrdenadorAutocomplete
 * Input con autocompletado fuzzy para ordenadores
 * Portal de Colaboradores GESTAR SALUD IPS
 */

import { useState, useCallback, useRef, useEffect } from 'react'
import { Search, X, ChevronDown } from 'lucide-react'
import { ORDENADORES_LISTA } from '@/types/back.types'

interface OrdenadorAutocompleteProps {
    value: string
...
function fuzzyMatch(query: string, text: string): boolean {
    ...
    // Todas las palabras del query deben aparecer en el texto
    return queryWords.every(word => normalizedText.includes(word))
}

export function OrdenadorAutocomplete({
    value,
    onChange,
    placeholder = 'Buscar ordenador...',
    disabled = false,
}: OrdenadorAutocompleteProps) {
    ...
    useEffect(() => {
        if (inputValue.trim().length >= 2) {
            const filtered = ORDENADORES_LISTA.filter(opt => fuzzyMatch(inputValue, opt))
            setFilteredOptions(filtered.slice(0, 10)) // Limitar a 10 resultados
        } else {
            setFilteredOptions([])
        }
    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
            if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
                setIsOpen(false)
                // Si el valor no está en la lista, mantenerlo igual (texto libre)
    ...
    }, [isOpen, highlightedIndex, filteredOptions, handleSelect, inputValue, onChange])

    const handleBlur = useCallback(() => {
        // Pequeño delay para permitir clicks en la lista
        setTimeout(() => {
            if (inputValue.trim() && inputValue !== value) {
                onChange(normalizeForSearch(inputValue))
            }
        ...
        <div ref={containerRef} className="relative">
            ...
            <div className="relative">
                ...
                <input
                    ...
                    disabled={disabled}
                    className={`
                        w-full pl-10 pr-16 py-2.5 rounded-lg border bg-white
            ...
            {/* Mensaje cuando no hay resultados pero hay búsqueda */}
            {isOpen && inputValue.length >= 2 && filteredOptions.length === 0 && (
                <div className="absolute z-50 w-full mt-1 p-3 bg-white border border-gray-200 rounded-lg shadow-lg">
                    <p className="text-sm text-gray-500 text-center">
                        ...
                        <br />
                        <span className="text-xs">Presiona Enter para usar el texto ingresado.</span>
                    </p>
                </div>
            )}
        </div>
...
export default OrdenadorAutocomplete
